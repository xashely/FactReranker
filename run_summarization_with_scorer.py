...
class CTTrainer(Seq2SeqTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        logits_mean = sum([val[-1].mean(-1).mean(-1) for val in beam_outputs['decoder_attentions']]) / len(
            beam_outputs['decoder_attentions'])
        logits = (logits_max + logits_mean + logits_min).mean(-1).reshape(batch_size,num_beams)
        logits = torch.softmax(logits.to(device=labels.device), dim=-1)
        self.logits.append(logits)

        seqs = self._pad_tensors_to_max_len(beam_outputs["sequences"], self.model.config.max_length)
        self.sequences.append(seqs)
        ori_seqs = self._pad_tensors_to_max_len(encoder_input_ids.repeat(num_beams, 1), 512)
        self.original_sequences.append(ori_seqs)
        # self.original_sequences.extend(tokenizer.batch_decode(np.repeat(encoder_input_ids, num_beans)))
    ...
    def calculate_contrastive_loss(self, num_beams, logits, sequences, labels, original_sequences):
        ...
        candidate_labels, scores = self.calculate_score(sequences, num_beams, labels, original_sequences)
        #print(labels.shape,beam_outputs["sequences"].shape)
        #print(encoder_outputs.shape)
        logits = F.log_softmax(logits.to(candidate_labels.device), dim=1)
        candidate_labels = F.log_softmax(candidate_labels, dim=1)
        contrastive_loss = nn.KLDivLoss(reduction='none')(logits,candidate_labels)
    ...
    def calculate_score(self, tokens, candidate_num, origin_tokens, input_tokens):
        overall_batch_size, max_sequence_length = tokens.shape
        batch_size = overall_batch_size // candidate_num
        
        decoded_tokens = self.tokenizer.batch_decode(tokens, skip_special_tokens=True)
        decoded_tokens = [v.strip() for v in decoded_tokens]
        input_tokens = self.tokenizer.batch_decode(input_tokens, skip_special_tokens=True)
        input_tokens = [v.strip() for v in input_tokens]
        ...
        #print(select_index)
        #print(select_tokens)
        return torch.softmax(torch.Tensor(np.asarray(scores).reshape(batch_size, candidate_num)), dim=-1), np.asarray(scores).reshape(batch_size, candidate_num)
...
def main():
    ...
    trainer = CTTrainer(
        ...
        compute_metrics=compute_metrics if training_args.predict_with_generate else compute_metrics,
    )
    results = {}
    max_length = (
