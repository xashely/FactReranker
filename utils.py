...
# ========== batch version ========= #
def ranking_fast(factual_hidden, context_hidden, next_hidden, next_top_k_probs, alpha, beam_width, beta):
    '''
    ...
    norm_factual_hidden = norm_factual_hidden / norm_factual_hidden.norm(dim=2, keepdim=True)
    factual_cosine_matrix = torch.matmul(norm_factual_hidden, norm_next_hidden.transpose(1, 2)).sum(-1)# [B*K]
    factual_scores, _ = torch.max(factual_cosine_matrix, dim=-1)  # [B*K]

    scores = (1.0 - alpha - beta) * next_top_k_probs - alpha * scores + beta * factual_scores
