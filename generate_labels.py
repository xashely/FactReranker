...
class CTTrainer(Seq2SeqTrainer):
    ...
    def compute_loss(self, model, inputs):
        ...
        return loss
        start = arrow.now()
        self.scorer = RadGraph(reward_level="partial", cuda=torch.cuda.current_device(), batch_size=1)

        encoder_input_ids = inputs['input_ids']
        #num_beams = self.model.config.num_beams
        num_beams = 3
        module_model = model.module
        encoder_outputs = original_outputs
        wandb.log({"train/encoder_cost": (arrow.now() - start).total_seconds()}, commit=False)
        batch_size, sequence_length, _ = encoder_outputs["encoder_last_hidden_state"].shape
        true_batch_size = batch_size // num_beams
        input_ids = torch.ones((batch_size, 1), device=module_model.device, dtype=torch.long)
        input_ids = input_ids * module_model.config.decoder_start_token_id
        model_kwargs = {
            "encoder_outputs": encoder_outputs,
        }
        beam_scorer = BeamSearchScorer(
            batch_size=true_batch_size, num_beams=num_beams, length_penalty=module_model.config.length_penalty, do_early_stopping=module_model.config.early_stopping, num_beam_hyps_to_keep=num_beams, device=module_model.device
        )
        logits_processor = LogitsProcessorList(
            [
                MinLengthLogitsProcessor(num_beams, eos_token_id=module_model.config.eos_token_id)
            ]
        )
        #print (encoder_outputs["last_hidden_state"].shape, input_ids.shape)
        beam_outputs = module_model.beam_search(input_ids, beam_scorer, logits_processor=logits_processor, pad_token_id=module_model.config.pad_token_id, eos_token_id=module_model.config.eos_token_id,
        ...
        wandb.log({"train/contrastive_loss": contrastive_loss.mean(), "train/original_loss": loss.mean().detach(), "train/loss_computing_cost": (arrow.now() - start).total_seconds()}, commit=False)
        ...
        return loss+0.2*contrastive_loss.to(loss.device)
    ...
    def calculate_contrastive_loss(self, encoder_outputs, num_beams, beam_outputs, labels):
        ...
        contrastive_loss = nn.CrossEntropyLoss(reduction='none')(logits, candidate_labels)
    ...
    def calculate_score(self, tokens, candidate_num, origin_tokens):
        ...
        decoded_origin_tokens = np.repeat(decoded_origin_tokens, candidate_num)
        ...
        scores = self.scorer(decoded_tokens, decoded_origin_tokens)[1]
        # scores = [
...
#        )
        ...
        # encoder_outputs = encoder_outputs.repeat_interleave(3, dim=0)
        # encoder_outputs = torch.mean(encoder_outputs.repeat_interleave(num_beams, dim=0), 1)
        # encoder_outputs = torch.mean(encoder_outputs, 1)
    ...
    def ct_search(self, model, input_ids, decoder_ids, **kwargs):
        ...
        logits = None
        input_embeds = None
        for step in range(kwargs["max_length"]):
