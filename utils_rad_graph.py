...
def postprocess_individual_report(file, final_dict, data_source=None):
    ...
    try:
        ...
        s = file["sentences"][0]
        temp_dict["entities"] = get_each_entity(n, r, s)
...
def get_entity(n, r, s):
    ...
    Returns:
        dict_entity: Dictionary containing the entities in the format similar to train.json
...
def get_each_entity(n,s):
    ...
    for idx, item in enumerate(n):
        start_idx, end_idx, label = item[0], item[1], item[2]
        entity.append(" ".join(s[start_idx: end_idx + 1]))
    return entity